/**
 * Extractor DIRECT para LatinUS (latinus.us) — lote "NEWS LAKE 200 FINAL PUSH"
 * (2026-07-22).
 *
 * LatinUS es B_PUBLICO_DIRECT: la homepage devuelve artículos en HTML estático
 * sin sitemap/RSS descubrible. El patrón de URL de artículo es:
 *   /SECCION/YYYY/M/DD/slug-NUMERO.html  (relativo → https://latinus.us/...)
 *
 ...
 *
 * ⚠ Nota de infraestructura: IPs de datacenter (GitHub Actions/AWS/GCP) pueden
 * recibir bloqueo de IPs o 403 de Cloudflare. Si el crawl falla en CI con 403/
 * conexión rechazada, documentar como "bloqueado en datacenter" y ejecutar solo
 * desde IP no-datacenter. Verificado OK desde entorno local 2026-07-22.
 *
 * Uso:
 *   npm run crawl-direct-latinus -- --dry-run
 *   npm run crawl-direct-latinus -- --limit=10
 */
...
export const MEDIO_ID = 'MED-0190';
export const BASE_URL = 'https://latinus.us';
export const HOME_URL = 'https://latinus.us/';

// Articulos con fecha en ruta: /seccion/YYYY/M/DD/slug-NUM.html (relativas)
...
async function main() {
  ...
  logger.info({ medio_id: MEDIO_ID, dryRun: args.dryRun, limit: args.limit }, 'Iniciando crawl-direct-latinus');
  ...
  logger.info({ enriquecidas }, '=== crawl-direct-latinus completado ===');
}
...
if (esEntrypointCli()) {
  main().catch((e) => { logger.error(e, 'Error fatal en crawl-direct-latinus'); process.exit(1); });
}
